...
public class TaleUtils {
  ...
  private static Properties getPropFromFile(String fileName) {
    ...
    try {
      // 默认是classPath路径
      InputStream resourceAsStream = TaleUtils.class.getResourceAsStream(fileName);
  ...
  public static String getFileKey(String name) {
    ...
    if (!new File(getWebSiteRootPath() + prefix).exists()) {
      new File(getWebSiteRootPath() + prefix).mkdirs();
    }
  ...
  public static String getWebSiteRootPath() {
    ...
    // LOGGER.info("upload file path 1: " + path);
    return path;
  }
}
